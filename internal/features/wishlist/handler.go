...
func (h *Handler) Replace(c *gin.Context) {
	...
	if err := h.svc.Replace(c.Request.Context(), uid, req); err != nil {
		response.Error(c, err)
		return
	}
	response.OK(c, nil)
...
func (h *Handler) Add(c *gin.Context) {
	...
	if productID == "" {
		response.Error(c, apperr.NewValidation("product_id is required"))
		return
	}
	if err := h.svc.Add(c.Request.Context(), uid, productID); err != nil {
		response.Error(c, err)
		return
	}
	response.OK(c, nil)
...
func (h *Handler) Remove(c *gin.Context) {
	...
	if err := h.svc.Remove(c.Request.Context(), uid, productID); err != nil {
		response.Error(c, err)
		return
	}
	response.OK(c, nil)
